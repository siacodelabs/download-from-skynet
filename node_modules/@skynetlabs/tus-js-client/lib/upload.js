...
class BaseUpload {
  constructor (file, options) {
    ...
    // If a partial upload, the current chunk object that this partial upload is
    // uploading.
    this._currentChunk = null

    // Whether this is an initial upload, or subtasks initiated in
  ...
  _startParallelUpload () {
    ...
    let totalProgress = 0
    const { chunkSize, staggerPercent } = this.options
    ...
    const uploads = parts.map((part, index) => {
      ...
      return this._source.slice(part.start, part.end)
        .then(({ value }) => new Promise((resolve, reject) => {
          ...
          // Finalize the partial upload fields.
          upload.options.onProgress = (newPartProgress) => {
            ...
            // Signal to the initial `BaseUpload` object.
            this._emitProgress(totalProgress, totalSize)

            // If a chunk stagger is set, signal to the other upload when it can
            // start uploading.
            if (staggerPercent !== null) {
              // Calculate stagger using the current chunk and not the
              // current part size.
              const chunk = upload._currentChunk
              const chunkProgress = (newPartProgress - (chunkSize * chunk.indexInPart))
              const chunkPercent = (chunkProgress / chunkSize) * 100
              const nextChunk = chunk.nextChunkStagger
          ...
          if (staggerPercent !== null) {
            const currentChunk = partsChunks[index][0]

            // Wait for the initial stagger for this chunk before starting the part.
            promise = currentChunk.promise

            upload._currentChunk = currentChunk
          }
          promise.then(() => upload.start())
  ...
  _handleUploadResponse (req, res) {
    ...
    promise.then(() => this._performUpload())
  }
...
function buildChunkStaggers (totalSize, partCount, chunkSize) {
  ...
  function newChunk (partsChunks, partIndex) {
    const chunksForPart = partsChunks[partIndex]

    let chunkResolve
    const promise = new Promise((resolve) => { chunkResolve = resolve })
    const chunk = {
      promise,
  ...
  // Validation.
  if (partCount < 1) {
    throw new Error(`Expected option 'partCount' to be greater than or equal to 1, was type 'number', value '${partCount}'`)
  }
  if (chunkSize < 1) {
    throw new Error(`Expected option 'chunkSize' to be greater than or equal to 1, was type 'number', value '${chunkSize}'`)
  }
  // NOTE: Unexpected code flow. `uploadLargeFileRequest` in skynet-js should
  // not enable parallel uploads for this case.
  if (totalSize <= chunkSize) {
    throw new Error(`Expected parameter 'totalSize' to be greater than the size of a chunk ('${chunkSize}'), was type 'number', value '${totalSize}'`)
  }
  ...
  for (let i = 0; i < numFullChunks; i++) {
    newChunk(partsChunks, i % partCount)
  }

  // Whether there is any leftover that has to go in the last part.
  const hasLeftover = (totalSize % chunkSize) > 0
  // If there is non-chunk-aligned leftover, add a chunk for it.
  if (hasLeftover) {
  ...
  let longestPartIndex = 0
  let longestPartLength = partsChunks[0].length
  for (let i = 1; i < partsChunks.length; i++) {
    if (partsChunks[i].length > longestPartLength) {
      longestPartIndex = i
      longestPartLength = partsChunks[i].length
    }
  }
  // Get the chunk at which to start uploading.
  const firstChunk = partsChunks[longestPartIndex][0]
  // Build the stagger sequence.
  let lastChunk = null
  // Loop over the chunk indices for the longest part.
  for (let i = 0; i < longestPartLength; i++) {
    ...
    // if we need to.
    for (let j = longestPartIndex; j < longestPartIndex + partCount; j++) {
      // Roll over to the first part if we need to with the modulo.
      const index = j % partCount

      if (partsChunks[index].length > i) {
        const chunk = partsChunks[index][i]
        // Last chunk in stagger order should point to this one.
        if (lastChunk !== null) {
